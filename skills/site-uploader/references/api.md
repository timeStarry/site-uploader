...
- `POST /api/sites`: authenticated multipart upload. Required parts are `title` and `file`; `summary` and `access_password` are optional. `access_password`, when present, must be exactly four ASCII digits. Files up to 64 MiB are accepted; larger files are rejected with `413` and are never silently truncated.
- `PUT /api/sites/{id}`: authenticated replacement upload for an existing ID. `file` is required; `title`, `summary`, and `access_password` are optional updates. Use this when the user asks to repair or replace an existing page while preserving its URL.
